...
(function ($) {
  ...
  $.widget("l5.collapsibleMenu", {
				...
				expand : function () {
					...
					if (! $head.hasClass('active')) {
						...
						if (! o.options.multiple) {
							...
							// CLOSE ALL OPEN
							o.headSet.filter('.active').not(this).trigger('collapse');
		...
		append : function ( head ) {
			
			// REMOVE UTILITY CLASSES
			this.headSet.removeClass('last');
			
			this._makeCollapsible( this.headSet.length, head );
			
			this.headSet = this.headSet.add(head);
		...
		next : function ( ) {
			...
			if ( ! this.options.multiple ) {
				var	index = this.headSet.index( this.headSet.filter('.active') ),
					current = this.headSet.filter(':eq(' + index + ')'),
					nextIndex = index + 1,
					next = this.headSet.filter(':eq(' + nextIndex + ')');

				// FIND A NONHIDDEN HEAD ITEM
				while (next.length > 0 && (next.is(':hidden') || next.hasClass('disabled'))) {
					nextIndex++;
					next = this.headSet.filter(':eq(' + nextIndex + ')');
				}
